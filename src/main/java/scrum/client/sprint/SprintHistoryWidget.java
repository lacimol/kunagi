...
public class SprintHistoryWidget extends AScrumWidget {
	...
	protected void onUpdate() {
		...
		List<Sprint> sprintsToView = sprints.subList(0, Math.min(sprints.size(), MAX_SPRINTS_IN_HISTORY_VIEW));
		if (changeIndicator.update(sprintsToView)) {
			page.clear();
			for (Sprint sprint : sprintsToView) {
				page.addHeader(sprint.getFullLabel());
